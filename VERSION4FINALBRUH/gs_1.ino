...
void prot9(String valor) {
  // Formato entrada: tiempo:X:Y:Z
  // Formato salida Python: "Position: (X: ... m, Y: ... m, Z: ... m)"
...
// === PROTOCOLO PANEL SOLAR (CORREGIDO) ===
void prot10(String valor) {
  // Formato entrada: porcentaje (0, 40, 60, 100)
  // Formato salida Python: "Panel:<porcentaje>"
  Serial.print("Panel:");
...
void setup() {
  Serial.begin(9600);
  mySerial.begin(9600);
  Serial.println("COMM LISTO orbital + panel solar");
  ...
  if (mySerial.available()) {
    ...
    if (data.length() > 0) {
      ...
      if (sepr > 0) {
        ...
        else if (id == 10) prot10(valor); // PANEL SOLAR
