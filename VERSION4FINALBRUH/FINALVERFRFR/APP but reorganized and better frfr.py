...
def detectar_puerto_automatico():
    ...
    # Lista de descripciones comunes de Arduino y microcontroladores
...
usbSerial = None #Declaración de usbSerial
device = detectar_puerto_automatico()

if device is None:
    # Si no se detecta automáticamente, usar el puerto manual
    comm_port = '13' #Cambiar este numero si la detección automática falla
...
#Para el grafica de sonar
latest_distance = 0
thetas = []
radios = []

#Para datos orbitales y ground track
orbit_x = []
orbit_y = []
orbit_z = []
...
transmission_state = "stopped" #Esto es para el color de los botones, segun si se esta manteniendo la comunicación
...
#DEPENDENCIAS
#Conversión a long/lat (necesario para prot_orbit())
...
#Inicio subfunciones protocolo
#Procesado de datos de orbita
...
def prot_solar(match_panel):
    ...
    try:
        ...
        if new_state != old_state:
            ...
            print(f"🛰️ {msg}")
            # Ejecutar messagebox en el hilo principal
...
def corrupt_chcksum(parts):
    ...
    try:
        ...
        pass
#Fin subfunciones protocolo

#Inicio función leer serial principal
def read_serial():
...
#Fin función leer serial principal
#Fin procesado de datos de entrada




#Funciones del programa
#Carga de eventos.txt
def cargar_eventos():
    evs = []
...
#Comprobación carga eventos.txt
if not os.path.exists(EVENTOS_FILE):
    try:
...
#Fin comprobación carga eventos.txt

#Definición funciones de botones
...
def abrir_ground_track():
    """Abre ventana separada con Ground Track en mapa de la Tierra"""
    ...
    # Crear figura más grande para ventana dedicada
    fig_gt_win, ax_gt_win = plt.subplots(figsize=(10, 7), facecolor='#0a0a2e')
    ax_gt_win.set_facecolor('#1a1a3e')
    
    # Intentar cargar imagen de mapa
    try:
        from matplotlib import image as mpimg
        # Buscar la imagen en la carpeta actual
    ...
    # Función de actualización para esta ventana
...
#Funciones de actualización de los graficos principales:
...
#2.4.2 Columna 3, panel:
panel_frame = Frame(events_frame, bg="navy")
panel_frame.pack(pady=10)
#Fin definición de los contenedores.



#Inicio Labels:
Label(orbit_frame, text="🌍 Órbita Satelital (2D)", font=subtitle_font, bg=bg_main, fg=tf).pack()
...
#Velocidad transmision (debe ir ANTES del botón validar)
...
#Funciones para el placeholder de observaciones
def on_obs_focus_in(event):
    """Elimina el placeholder cuando se hace clic"""
