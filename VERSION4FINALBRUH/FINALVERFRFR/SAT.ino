...
double eccentricity = 0.05;  // ejemplo: 0.05 = 5% elíptica
...
void compute_orbit(uint16_t &time_s_out, int32_t &x_out, int32_t &y_out, int32_t &z_out) {
    ...
    double inclination = PI/8;  // misma que antes
    int ecef = 0;              // mismo que antes

    // === Órbita elíptica con excentricidad ===
    double a = r;               // semieje mayor
    double b = a * sqrt(1 - eccentricity * eccentricity);  // semieje menor
    double x = a * cos(angle);
    double y = b * sin(angle);
    double z = 0;  // si quieres inclinación, puedes poner z = y*sin(inclination)
