...
const unsigned long TOKEN_TIMEOUT = 6000; // CORREGIDO
...
void sendPacketWithChecksum(uint8_t type, const String &payload) {
  ...
  Serial.println("-> " + fullMsg);     // cifrado/entregado
  Serial.println("PLAIN-> " + msg);    // para debug local (opcional)
}
...
void handleCommand(const String &cmd) {
    Serial.println("RX cmd (plain): " + cmd);
    ...
    if (cmd.startsWith("1:")) {
        ...
        sendPacketWithChecksum(67, "0"); // ack opcional
    }
    else if (cmd.startsWith("2:")) {
        // establecer ANGULO MANUAL via 2:ang
        ...
        autoDistance = false;
        // En modo manual nos movemos al ángulo guardado
        motor.write(manualTargetAngle);
        servoAngle = manualTargetAngle;
        ...
        // establecer ANGULO MANUAL (5:ang)
        ...
        // CORRECCIÓN CRUCIAL: si estamos en modo MANUAL, mover servo inmediatamente
        if (!autoDistance) {
            motor.write(manualTargetAngle);
            servoAngle = manualTargetAngle;
        } else {
            // si estamos en auto, simplemente actualizar target (no mover motor manualmente)
            servoAngle = manualTargetAngle; // mantenerse consistente
        }
        // confirmar ángulo recibido
        sendPacketWithChecksum(6, String(manualTargetAngle));
    }
}
...
void setup() {
    ...
    Serial.println("SAT listo con orbital (MANUAL FIXED)");
}

void loop() {
    unsigned long now = millis();
    
    // Servo automático (solo si autoDistance == true)
    ...
    // Recuperación por timeout del token
