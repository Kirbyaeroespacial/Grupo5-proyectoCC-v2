...
const uint8_t potent = A0;
unsigned long lastReceived = 0;
unsigned long last = 0;
const unsigned long timeout = 8000; // CORREGIDO
const unsigned long delay_ang = 1500; // CORREGIDO
...
void sendWithChecksum(const String &msg) {
  ...
  Serial.println("GS-> " + fullMsg); // debug
}
...
void setup() {
  ...
  lastReceived = millis(); // evitar timeout inicial
}

void loop() {
  unsigned long now = millis();

  // Gestión de turnos: otorgar token periódicamente
  ...
  // Comandos del usuario (vía Serial USB). Si ya contienen '*', se reenvían tal cual.
  if (Serial.available()) {
    ...
    if (command.length() > 0) {
      if (command.indexOf('*') != -1) {
        ...
        Serial.println("GS-> " + command); // debug
      } else {
  ...
  // Envío periódico del ángulo del potenciómetro con checksum
  ...
  // Recepción desde el satélite con validación
  if (mySerial.available()) {
    ...
    if (data.length() > 0) {
      ...
      Serial.println("SAT-> OK: " + cleanMsg); // debug (mensaje descifrado)
  ...
  // Timeout: si hace mucho que no llegan datos del satélite
