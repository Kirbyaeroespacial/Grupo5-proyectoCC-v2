...
#define SHIFT_KEY 7   // <<< CLAVE DE CIFRADO: mantener idéntica en el satélite

int errpin = 2;
char potent = A0;
unsigned long lastReceived = 0;
unsigned long last = 0;
const unsigned long timeout = 8000; // CORREGIDO: de 5000 a 8000
const unsigned long delay_ang = 1500; // CORREGIDO: de 200 a 1500
...
// --- CIFRADO / DESCIFRADO (soporta A-Z, a-z, 0-9) ---
String encryptString(const String &s, int N) {
  String out = "";
...
String decryptString(const String &s, int N) {
  String out = "";
  for (unsigned int i = 0; i < s.length(); i++) {
    char c = s[i];
    if (c >= 'A' && c <= 'Z') {
      char r = ((c - 'A' - N + 26) % 26) + 'A';
      out += r;
    } else if (c >= 'a' && c <= 'z') {
      char r = ((c - 'a' - N + 26) % 26) + 'a';
      out += r;
    } else if (c >= '0' && c <= '9') {
      char r = ((c - '0' - N + 10) % 10) + '0';
      out += r;
    } else {
      out += c;
    }
...
// Envía mensaje: cifra antes de checksum
void sendWithChecksum(const String &msg) {
  String enc = encryptString(msg, SHIFT_KEY);
  String chk = calcChecksum(enc);
  String fullMsg = enc + "*" + chk;
  mySerial.println(fullMsg);
  Serial.println("GS-> (ENCRYPTED) " + fullMsg); // debug
  Serial.println("GS-> (PLAIN) " + msg);
}

// Valida y descifra; devuelve true si OK y pone cleanMsg descifrado
bool validateMessage(const String &data, String &cleanMsg) {
  int asterisco = data.indexOf('*');
  if (asterisco == -1) return false;
  
  String enc = data.substring(0, asterisco);
  String chkRecv = data.substring(asterisco + 1);
  String chkCalc = calcChecksum(enc);
  
  if (chkRecv != chkCalc) return false;
  
  // descifrar
  cleanMsg = decryptString(enc, SHIFT_KEY);
  return true;
}
...
void setup() {
  Serial.begin(9600);
  mySerial.begin(9600);
  Serial.println("COMM LISTO orbital (ENCRYPTION ON)");
...
void loop() {
  ...
  if (!satHasToken && now - lastTokenSent > TOKEN_CYCLE) {
    sendWithChecksum("67:1"); // envía cifrado
    satHasToken = true;
    lastTokenSent = now;
  }
  ...
  if (Serial.available()) {
    ...
    command.trim();
    if (command.length() > 0) {
      // Si ya tiene '*' lo enviamos tal cual (compatibilidad)
      if (command.indexOf('*') != -1) {
        mySerial.println(command);
        Serial.println("GS-> (RAW SEND) " + command); // debug
      } else {
        // añadimos cifrado+checksum
  ...
  if (mySerial.available()) {
    ...
    if (data.length() > 0) {
      ...
      if (!validateMessage(data, cleanMsg)) {
        Serial.println("SAT-> CORRUPTO: " + data); // debug
        corruptedFromSat++;
      ...
      Serial.println("SAT-> OK: " + cleanMsg); // ahora ES EL MENSAJE DESCIFRADO
