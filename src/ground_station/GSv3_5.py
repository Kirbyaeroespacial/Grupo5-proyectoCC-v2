...
# Setup del serial - CAMBIAR PUERTO AQUÍ
device = 'COM7'  # Windows: COM3, COM4, etc. | Linux/Mac: /dev/ttyUSB0, /dev/ttyACM0
try:
    usbSerial = serial.Serial(device, 9600, timeout=1)
    print(f"✓ Puerto {device} abierto correctamente")
except Exception as e:
    print(f"⚠ ERROR: No se puede abrir {device}")
    print(f"Error: {e}")
    exit(1)
...
# === CHECKSUM (MISMO ALGORITMO QUE ARDUINO) ===
def calc_checksum(msg):
    """Calcula checksum XOR en hexadecimal - IGUAL QUE ARDUINO"""
    xor_sum = 0
    for char in msg:
        xor_sum ^= ord(char)
...
def read_serial():
    ...
    while True:
        ...
        # Actualizar tiempo para cualquier mensaje
        last_data_received = time.time()

        # Mensajes de debug del ground station
        if linea.startswith("TX>") or linea.startswith("RX<") or linea.startswith("===") or linea.startswith("⚠") or linea.startswith("✓"):
            print(f"[GND] {linea}")
            continue

        # Chequear si es posición orbital (sin checksum)
        ...
        try:
            if len(parts) >= 2:
                ...
                elif idn == '67':  # Token (ignorar, ya manejado)
                    pass

                elif idn == '99':  # Estadísticas corrupción
                    try:
