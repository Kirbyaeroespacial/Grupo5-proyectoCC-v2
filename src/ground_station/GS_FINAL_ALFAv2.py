...
# <- Cambio: ahora latest_data incluye "avgt" para la temperatura media recibida
latest_data = {"temp": 0, "hum": 0, "avgt": 0}
latest_distance = 0  # en mm
angulo = 90          # inicializamos ángulo por defecto (grados)

# <- Cambio: temp_med ahora es un deque (igual que temps/hums) para plotear una serie
temp_med = deque([0]*max_points, maxlen=max_points)
...
def read_serial():
    ...
    while True:
        ...
        parts = linea.split(':')
        try:
            # <- Cambio mínimo: incluir '7' y '8' en la lista de IDs aceptados
            if len(parts) >= 2 and parts[0] in ('1','2','3','4','5','6','7','8'):
                ...
                if idn == '1':
                    ...
                    # <- Cambio: guardar la media recibida en latest_data["avgt"]
                    try:
                        latest_data["avgt"] = int(parts[1]) / 100.0
                        # opcional: ver en consola
                        print(f"Media recibida: {latest_data['avgt']:.2f} °C")
                    except ValueError:
                        pass
                elif idn == '8':
...
# <- Cambio: crear correctamente la línea de media con un deque inicial
...
def update_plot():
    global temp_med
    # añadir nuevos valores recibidos
    temps.append(latest_data["temp"])
    hums.append(latest_data["hum"])
    # <- Cambio: añadir la media recibida (latest_data["avgt"]) al deque temp_med
    ...
    # actualizar eje x acorde a la longitud actual (deques mantienen max_points)
    x = range(len(temps))
    line_temp.set_xdata(x)
    line_hum.set_xdata(x)
    line_med.set_xdata(x)
