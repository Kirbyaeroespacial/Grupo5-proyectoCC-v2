...
const unsigned long timeout = 8000; // Timeout aumentado
const unsigned long delay_ang = 200;

// Gestión de turnos MEJORADA
bool satHasToken = false;
unsigned long lastTokenSent = 0;
const unsigned long TOKEN_CYCLE = 3500; // Sincronizado con heartbeat del satélite
...
void loop() {
  ...
  // IMPORTANTE: Solo enviar si el satélite NO tiene el token (para evitar colisiones)
  if (Serial.available()) {
    String command = Serial.readStringUntil('\n');
    command.trim();
    if (command.length() > 0) {
      // Esperar un momento si el satélite está transmitiendo
      if (satHasToken) {
  ...
  if (now - lastReceived > timeout) {
    Serial.println("TIMEOUT - Sin señal del satélite");
    digitalWrite(errpin, HIGH);
    delay(100);
    digitalWrite(errpin, LOW);
    delay(50);
