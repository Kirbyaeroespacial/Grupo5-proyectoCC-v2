...
# NO OLVIDAR DESCOMENTAR AL PROBAR!!!! (ya está abierto aquí)
...
# --------------------
# Funciones checksum integradas (mínimo cambio)
# --------------------
def checksum_bytes(s: str) -> int:
    """Suma de bytes (UTF-8) % 256 — igual que en Arduino."""
    b = s.encode('utf-8', errors='replace')
    return sum(b) & 0xFF

def comprobar_linea_con_checksum(line: str):
    """
    Si la línea tiene formato payload|CHK, devuelve (payload, chk_recv:int, chk_calc:int, ok:bool).
    Si no tiene '|', devuelve (None, None, None, False).
    """
    line = line.rstrip('\r\n')
    if '|' not in line:
...
# Ahora espera líneas en formato: <payload>|<CHK>
# --------------------
def read_serial():
    ...
    while True:
        ...
        try:
            ...
            linea = raw.decode('latin1', errors='replace').strip()

        if not linea:
            time.sleep(0.01)
            continue

        # Validar checksum: la línea debe venir como "payload|CHK"
        payload, chk_rec, chk_calc, ok = comprobar_linea_con_checksum(linea)
        if payload is None:
            # Si no hay separador '|', asumimos formato antiguo o inválido -> ignorar para evitar procesar basura
            # Si quieres compatibilidad retroactiva, puedes procesar la línea sin checksum aquí,
            # pero según tu petición, descartamos hasta que el protocolo incluya checksum.
            print(f"Ignorado (sin checksum): '{linea}'")
            time.sleep(0.01)
            continue

        if not ok:
            # Línea con checksum mal formado o que no coincide
            if chk_rec is None:
                print(f"Ignorado (chk no numérico): '{linea}'")
            else:
                print(f"Ignorado (checksum mismatch). Recibido={chk_rec} Calculado={chk_calc}. Línea='{linea}'")
            time.sleep(0.01)
            continue

        # Si checksum OK, procesar payload (formato esperado: id:... como antes)
        try:
            parts = payload.split(':')
            if len(parts) >= 2 and parts[0] in ('1','2','3','4','5','6','7','8'):
                ...
                # Compatibilidad antigua: si deseas procesar líneas legacy,
                # añade aquí el comportamiento antiguo. Por seguridad lo dejamos ignorado.
                print(f"Ignorado (payload formato desconocido): '{payload}'")
        except Exception as e:
            print("Parse error:", e)

        time.sleep(0.01)

# Lanzar hilo lector (daemon)
...
def update_radar():
    ...
    # print(angulo)   # evita spam en consola; descomenta si quieres traza
