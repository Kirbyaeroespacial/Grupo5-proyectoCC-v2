...
device = 'COM7'
try:
    usbSerial = serial.Serial(device, 9600, timeout=0.5)
    print(f"✓ Conectado a {device}")
except serial.SerialException as e:
    print(f"⚠ Error abriendo puerto {device}: {e}")
    print("Por favor, cambia 'device' en el código y reinicia")
    exit(1)
...
def validate_message(line):
    ...
    if '*' not in line:
        # Mensaje sin checksum - descartar
        total_corrupted += 1
        print(f"⚠ Mensaje sin checksum descartado (Total corrupted: {total_corrupted})")
        return False, ""
    ...
    msg = parts[0]
    recv_checksum = parts[1].strip().upper()
    calc_check = calc_checksum(msg).upper()
    
    if recv_checksum == calc_check:
        return True, msg
    else:
        total_corrupted += 1
        print(f"⚠ Checksum inválido: esperado {calc_check}, recibido {recv_checksum} (Total: {total_corrupted})")
        return False, ""
...
def read_serial():
    """Lee datos del puerto serial con validación MEJORADA"""
    ...
    while True:
        try:
            ...
            print(f"⚠ Error leyendo serial (¿Arduino desconectado?): {e}")
            time.sleep(1)
            continue
        ...
        # Actualizar tiempo de última recepción para cualquier mensaje válido
        last_data_received = time.time()

        # Mensajes de debug del ground station (sin checksum)
        if linea.startswith("->") or linea.startswith("<-") or linea.startswith("===") or linea.startswith("⚠") or linea.startswith("✓"):
            # Estos son mensajes de debug, no actualizar last_data_received aquí
            print(f"[GND] {linea}")
            time.sleep(0.01)
            continue

        # Chequear si es posición orbital (viene del ground station parseada)
        ...
        # MEJORADO: Validar TODOS los mensajes con datos
        if ':' in linea:
            # Si tiene checksum, validar
            if '*' in linea:
                valid, clean_msg = validate_message(linea)
                if not valid:
                    # Ya se incrementó total_corrupted en validate_message
                    time.sleep(0.01)
                    continue
                linea = clean_msg
            else:
                # Mensaje con ':' pero sin checksum - descartar
                total_corrupted += 1
                print(f"⚠ Mensaje de datos sin checksum descartado: {linea[:50]}")
                time.sleep(0.01)
                continue
                ...
                if idn == '1':
                    if len(parts) >= 3:
                        try:
                            ...
                            print(f"⚠ Error parseando temp/hum: {linea}")
                    ...
                    try:
                        ...
                        print(f"⚠ Error parseando distancia: {linea}")
                    ...
                    try:
                        ...
                        print(f"⚠ Error parseando temp media: {linea}")

                elif idn == '8':
                    messagebox.showwarning("⚠ Alta temperatura!", "¡PELIGRO! Temp media >100°C")

                elif idn == '67':
                    # Mensajes de token (ya manejados por Arduino)
                    pass
                    ...
                    try:
                        ...
                        print(f"📊 [CHECKSUM] Descartados en Ground: {corrupted} | Total acumulado: {total_corrupted}")
                    except ValueError:
                        pass

        except Exception as e:
            print(f"⚠ Parse error en línea '{linea}': {e}")
...
# Label de errores de checksum
checksum_label = Label(title_frame, text="Errores: 0", font=("Inter", 10), 
                       bg="#1e1e2f", fg="#ff6b6b")
checksum_label.pack(side=LEFT, padx=20)

def update_status_label():
    """Actualiza el label de estado y errores"""
    with connection_lock:
        status_label.config(text=connection_status)
    checksum_label.config(text=f"Errores: {total_corrupted}")
...
print(f"Conectado a: {device}")
print("Esperando datos del satélite...")
print("="*50 + "\n")
