...
# === CHECKSUM PYTHON → compatible con Arduino ===
...
def send_cmd(msg: str):
    """Envía comando al Arduino con checksum correcto"""
...
# Estadísticas checksum (solo para visualizar)
total_corrupted = 0
...
def read_serial():
    ...
    while True:
        ...
        parts = linea.split(':')
        try:
            if len(parts) >= 2 and parts[0] in ('1','2','3','4','5','6','7','8','67','99','9'):
                idn = parts[0]
...
class VentanaOrbital:
    def __init__(self, parent):
        ...
        R_EARTH = 6371000
        earth = plt.Circle((0, 0), R_EARTH, color='orange', fill=False, linewidth=2)
    ...
    def update_plot(self):
        ...
        with orbit_lock:
            if len(orbit_x) > 0:
                ...
                max_coord = max(max(abs(x) for x in orbit_x), 
...
Label(title_frame, text="Control Satélite", font=title_font, 
      bg="#1e1e2f", fg="#ffffff").pack(side=LEFT, padx=20)
...
Button(title_frame, text="🛰️ Ver Órbita", font=("Inter", 12, "bold"),
       command=open_orbital, bg="#6b8dd6", fg="white",
       bd=0, padx=15, pady=8).pack(side=LEFT)
