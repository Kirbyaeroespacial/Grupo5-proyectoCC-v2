...
// Periodo de envío (puede cambiar mediante 1:<ms>)
...
int manualTargetAngle = 90;    // objetivo manual si se recibe 2:<valor>
...
void handleReceivedPacket(const String &cmd) {
    if (cmd.startsWith("1:")) { // Cambiar periodo de envío
        long ms = cmd.substring(2).toInt();
        if (ms >= 200 && ms <= 10000) sendPeriod = (unsigned long)ms;
    } 
    else if (cmd.startsWith("2:")) { // Nueva orientación del sensor
        int ang = cmd.substring(2).toInt();
        manualTargetAngle = constrain(ang, 0, 180);
        if (!autoDistance) {
