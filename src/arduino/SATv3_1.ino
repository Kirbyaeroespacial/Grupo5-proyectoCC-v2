...
// Checksum: contador de comandos corruptos recibidos
...
// === VALIDACIÓN con checksum antes de ejecutar comando ===
void validateAndHandle(const String &data) {
    ...
    if (chkRecv == chkCalc) {
        handleCommand(msg);  // Checksum OK, ejecuta
    } else {
...
void setup() {
    ...
    lastTokenTime = millis();
    Serial.println("SAT listo con checksum");
}
...
void loop() {
    ...
    // Leer comandos CON validación de checksum
