...
void setup() {
    ...
    digitalWrite(LEDPIN, LOW); // asegurar LED apagado al iniciar
...
void loop() {
    ...
    if (now - lastSend >= sendPeriod) {
        ...
        // Encender LED brevemente para indicar envío
        digitalWrite(LEDPIN, HIGH);
        ledState = true;         // <-- cambio: marcamos que está encendido
        ledTimer = now;
        lastSend = now;
    }

    // Apagar LED tras 80 ms desde el parpadeo
